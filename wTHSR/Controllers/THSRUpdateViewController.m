...
- (NSString *)tableView:(UITableView *)tv titleForFooterInSection:(NSInteger)section {
    ...
    if ( [self.items count] == section){
        ...
        tempHeader = @"\n\n更新高鐵時刻表　　　　　　\n\n請選擇您所需要的時刻表資料\n點選後立即更新時刻表\n";
        tempHeader = NSLocalizedString(@"UpdateFooter", @"");
        return tempHeader;
    }
...
{
    //    NSString    *URLString = @"https://s3.amazonaws.com/w-e-n-g/thsr.update.test.json";
    NSString	*URLString	= @"https://s3.amazonaws.com/w-e-n-g/thsr_update.json";
...
- (void)updateNow
{
    alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Downloading....", @"")
                                       message:@" "
                                      delegate:self
                             cancelButtonTitle:nil
...
- (void)updateEnd
{
    ...
    NSIndexPath *tableSelection = [myTableView indexPathForSelectedRow];
    [myTableView deselectRowAtIndexPath:tableSelection animated:NO];
    self.title = NSLocalizedString(@"Updated!", @"");
